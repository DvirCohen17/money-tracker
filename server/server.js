...
const app=express();
const db=new Database(process.env.DB_PATH || path.join(__dirname,'moneytracker.db'));
const JWT_SECRET=process.env.JWT_SECRET||'CHANGE_ME_IN_PRODUCTION';
app.use(cors());app.use(express.json({limit:'4mb'}));
// The same Node service can serve the static MoneyTracker app when deployed to a host such as Render/Railway/Fly.
...
app.post('/api/login',(req,res)=>{const {email,password}=req.body||{};const u=db.prepare('SELECT id,name,email,password_hash FROM users WHERE email=?').get(String(email||'').toLowerCase());if(!u||!bcrypt.compareSync(password||'',u.password_hash))return res.status(401).json({error:'invalid credentials'});res.json({token:jwt.sign({id:u.id,name:u.name,email:u.email},JWT_SECRET,{expiresIn:'30d'}),user:{id:u.id,name:u.name,email:u.email}})});
